...
class SequentialDataset(Dataset, FbankFeatLabelParser):
    def __init__(self, args, data_dir, dict_file, type_data):
        ...
        if self.feat_type.split('_')[0] == 'kaldi':
            self.speech_feats_list = self.loading_mix_feat_files(data_dir)
        ...
        self.spe_ids = []
        for speech_scp in self.speech_feats_list:     
            with open(speech_scp, encoding='utf-8') as f: #打开speech_scp 存储feates
                spe_ids = f.readlines()  #每行返回一个列表
            self.spe_ids.append([x.strip().split(' ') for x in spe_ids])  #按空格分出来strip去掉空格
        self.spe_size = len(self.spe_ids[0])  #音频文件个数
        
        if not os.path.exists(self.feat_len_scp):
            self.loading_feat_len(self.speech_feats_list[0], self.feat_len_scp)
        ...
        audio_lengths = [self.load_audio_feat_len(utt_id) for utt_id in self.spe_ids[0]]
    ...
    def loading_mix_feat_files(self, data_dir):
        ...
        print("There are "+str(len(mix_feats_list))+" noisy sets")
        return mix_feats_list
    ...
    def loading_cmvn(self):
        ...
        cmvn_name = self.type_data+'_'+'_cmvn.npy'
    ...
    def compute_cmvn(self):
        ...
        for n in tqdm(range(cmvn_num)):  
            audio_path = self.spe_ids[0][cmvn_rand_idx[n]][1]
            if self.feat_type.split('_')[0] == 'kaldi':
    ...
    def __getitem__(self, index):
        feats_num = len(self.spe_ids)
        idx = random.randint(0,feats_num-1)
        utt_key, utt_path = self.spe_ids[idx][index]
        ...
        if utt_key in self.utt2spk_ids:
            ...
            spk_id = self.ids_dict[self.utt2spk_ids[clean_utt_key]]
