...
class MixSequentialDataset(Dataset, FbankFeatLabelParser):
    def __init__(self, args, data_dir, dict_file,type_data):
        ...
        if self.feat_type.split('_')[0] == 'kaldi':
            ...
            self.mix_feats_list, self.mix_angle_list = self.loading_mix_feat_files(data_dir)
        ...
        self.mix_feat_ids = []
        for mix_feat_scp in self.mix_feats_list:
            with open(mix_feat_scp, encoding='utf-8') as f:
                utt_ids = f.readlines()
            self.mix_feat_ids.append([x.strip().split(' ') for x in utt_ids])
        self.mix_feat_size = len(self.mix_feat_ids[0])
        self.mix_angle_ids = []
        self.mix_angle_ids_dict = []
        for mix_angle_scp in self.mix_angle_list:
            with open(mix_angle_scp, encoding='utf-8') as f:
                utt_ids = f.readlines()
            self.mix_angle_ids.append([x.strip().split(' ') for x in utt_ids])
            self.mix_angle_ids_dict.append({x.strip().split(' ')[0]: x.strip().split(' ')[1] for x in utt_ids})
        
        if not os.path.exists(self.mix_feat_len_scp):
            self.loading_feat_len(self.mix_feats_list[0], self.mix_feat_len_scp)
        ...
        audio_lengths = [self.load_audio_feat_len(utt_id) for utt_id in self.mix_feat_ids[0]]
        ...
        for n in range(self.mix_feat_size):
            wav_path = self.mix_feat_ids[0][n][1]
            if self.feat_type.split('_')[0] == 'kaldi':
        ...
        ids = set(value for key, value in self.utt2spk_ids.items())
        self.ids_dict = {id:i for i,id in enumerate(ids)}
        ids = list(ids)
    ...
    def loading_mix_feat_files(self, data_dir):
        ...
        for item in file_list:
            ...
            if (os.path.isfile(file_path)):
                if "mix_feats" in item:
                    mix_feats_list.append(os.path.join(data_dir,item))
                if "mix_angles" in item:
                    mix_angle_list.append(file_path)
        if len(mix_feats_list) != len(mix_angle_list):
            raise Exception("mix feats or mix angles file error")
        mix_feats_list = sorted(mix_feats_list,key = lambda x: x.split('.')[-2][-1])
        mix_angle_list = sorted(mix_angle_list,key = lambda x: x.split('.')[-2][-1])
        return mix_feats_list, mix_angle_list
    ...
    def compute_cmvn(self):
        ...
        for n in tqdm(range(cmvn_num)):  
            audio_path = self.mix_feat_ids[0][cmvn_rand_idx[n]][1]
            if self.feat_type.split('_')[0] == 'kaldi':
    ...
    def __getitem__(self, index):
        num_feats = len(self.mix_feats_list)
        idx = random.randint(0,num_feats-1)                   
        mix_utt_id, mix_utt_path = self.mix_feat_ids[idx][index]
        if self.feat_type.split('_')[0] == 'kaldi':
            spect = self.extract_kaldi_feat(mix_utt_path, feat_type=self.feat_type)
            angle = self.extract_kaldi_feat(self.mix_angle_ids_dict[idx][mix_utt_id])
            spect[spect <= 1e-7] = 1e-7
        ...
        spk_id = self.ids_dict[self.utt2spk_ids[clean_utt_id]]
        ...
        return mix_utt_id, spk_id, clean_spect, clean_log_spect, mix_spect, mix_log_spect, cos_angle, target_out, clean_angle, mix_angle, self.cmvn[0]
